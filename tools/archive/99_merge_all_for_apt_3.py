...
<head>
  ...
  <style>
    ...
    .matrix-table th, .matrix-table td {{
      border: 1px solid #dde3ef; padding: 11px 7px; text-align: left;
      min-width: 140px;
    }}
    .matrix-table th {{
      background: #eaf0fa; color: #222b44;
      font-size: 1.09em;
      font-weight: 600;
    ...
    .matrix-table td {{
      background: #f9fbfd; vertical-align: top;
      min-width: 140px;
    }}
    .matrix-technique {{
      margin-bottom: 10px; padding: 10px 8px 10px 14px;
      border-radius: 7px; border: 1.2px solid #e5e9f2;
      background: #fff; box-shadow: 0 1.5px 7px #dde3ef33;
      position: relative;
      transition: box-shadow 0.13s;
    }}
    .matrix-technique:hover {{
      box-shadow: 0 2px 13px #8cc2ff33;
      border-color: #b3d3ff;
    }}
    ...
    .badge {{
      ...
      font-size: 0.93em; color: #fff; font-weight: 500;
    ...
    .tactic-icon {{
      width: 17px; height: 17px; vertical-align: text-bottom; margin-right: 4px;
      opacity: 0.82;
    }}
    .tactic-header {{
...
def main():
    ...
    # Wylistuj istniejące foldery APT do wyboru (dodatek)
    ...
    techs = []
    while True:
        tid = input("Podaj ID techniki (np. T1059): ").strip()
        if not tid:
            break
        tname = input("Podaj nazwę techniki: ").strip()
        tactics = input("Podaj taktyki (rozdzielone przecinkami): ").strip()
        status = ""
