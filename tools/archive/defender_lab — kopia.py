...
def generate_heatmap_section(apt_folder, techniques_db):
    """Ładuje tools/helpers/last30days_alerts.csv i generuje heatmapę na koniec raportu."""
    ...
    if not alerts or "AttackTechniques" not in alerts[0]:
        return "<h2>Heatmapa aktywności</h2><p><i>Brak danych do wygenerowania heatmapy.</i></p>"

    # Zlicz wystąpienia technik (oddzielone przecinkiem)
    counts = Counter()
    for row in alerts:
        if "AttackTechniques" in row:
            for tid in row["AttackTechniques"].split(","):
                tid = tid.strip().upper()
                if tid:
                    counts[tid] += 1
    ...
    # Przygotuj mapę technik tylko z tej matrycy (APT lub Single)
    mapped_techniques = set()
    for t in os.listdir(os.path.join("scenarios", apt_folder)):
        mapped_techniques.add(t.upper())
    # (jeśli nie znajdzie folderu scenarios, nie filtrujemy – zostawiamy wszystko)

    rows = []
    rows.append('<h2>🔥 Heatmapa aktywności z Defendera (ostatnie 30 dni)</h2>')
    rows.append('<div style="overflow-x:auto;max-width:1100px;">')
    rows.append('<table style="border-collapse:collapse;min-width:700px;font-size:1.07em;">')
    rows.append('<tr><th>Technika</th><th>Nazwa</th><th>Liczba alertów</th></tr>')
    for tid, cnt in sorted(counts.items(), key=lambda x: -x[1]):
        # jeśli heatmapa ma być tylko dla danej matrycy:
        if mapped_techniques and tid not in mapped_techniques:
            continue
        name = techniques_db.get(tid, {}).get("name", "(brak nazwy)")
        color = get_heatmap_color(cnt)
        rows.append(f'<tr style="background:{color};"><td><b>{tid}</b></td><td>{name}</td><td style="font-weight:bold;text-align:center;">{cnt}</td></tr>')
...
def generate_html_report(apt_folder, status_csv_path, techniques_db):
    ...
    html.append('</tr><tr>')
    # Zmapowane techniki per taktyka (do macierzy)
    ...
    for r in rows:
        for t in r["Tactics"].split(","):
            ...
            if key in matrix:
                ...
                cell = (
                    f'<div class="matrix-technique badge-{r["Status"]}" data-status="{r["Status"]}">'
...
def workflow():
    ...
    # Update (masowa aktualizacja raportów)
    elif mode == 3:
