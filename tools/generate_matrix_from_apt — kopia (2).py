...
def show_group_aliases(stix_path):
    ...
    print("-----------------------------------------------")
    try:
        wybor = int(input("Podaj numer z listy przypisany do grupy(np. 152): ")) - 1
        if 0 <= wybor < len(group_list):
            aliases = group_list[wybor]["aliases"]
            if aliases:
                print(f"Aliasów dla {group_list[wybor]['name']} ({group_list[wybor]['mitre_id']}): {', '.join(aliases)}")
            else:
                print("Brak aliasów dla tej grupy.")
        else:
            print("Nieprawidłowy numer grupy.")
    except Exception:
        print("Błąd: Podaj poprawny numer grupy.")
