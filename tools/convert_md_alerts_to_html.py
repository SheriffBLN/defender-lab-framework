...
ENTERPRISE_CSV = "enterprise_attack.csv"

def load_technique_to_tactics(csv_path):
    mapping = {}
    ...
    with open(csv_path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            tactics = row.get("Tactic") or row.get("Tactics") or ""
            if tid and tactics:
                mapping[tid.strip()] = [t.strip() for t in tactics.split(",")]
    return mapping
...
def clean_body_main(body_main):
    """Usuwa linie zaczynające się od 'Technique ID:' lub puste powtórki."""
    lines = body_main.splitlines()
    cleaned = [line for line in lines if not line.strip().startswith("Technique ID:")]
    return "\n".join(cleaned)
...
def generate_html_card(tid, tname, tactic, status, mitre_link, body_main, mitre_desc, author):
    body_main_clean = md_to_html_basic(clean_body_main(body_main)) if body_main else "<i>(brak)</i>"
...
<head>
  ...
  <style>
    ...
    .meta {{ font-size: .95em; color: #555; margin-bottom: .8em; background: #f2f2f2; border-radius: 8px; padding: 0.4em 1em; display: inline-block; }}
...
<div class="card">
  ...
  <div class="desc section"><b>Scenario / Custom Description:</b><br>{body_main_clean}</div>
...
def convert_md_alerts_to_html(alerts_folder, enterprise_csv=ENTERPRISE_CSV):
    print("Ładuję mapping technika → taktyki z enterprise_attack.csv...")
    technique_to_tactics = load_technique_to_tactics(enterprise_csv)
    for root, dirs, files in os.walk(alerts_folder):
        for filename in files:
            if filename.endswith(".md"):
                ...
                # Fallback: jeżeli tactics jest puste, pobierz z enterprise_attack.csv
                if (not tactic) and tid in technique_to_tactics:
                    tactic = ", ".join(technique_to_tactics[tid])
                html_card = generate_html_card(tid, tname, tactic, status, mitre_link, body_main, mitre_desc, author)
                with open(html_path, "w", encoding="utf-8") as f:
                    f.write(html_card)
    print("[✓] Wszystkie pliki .md przekonwertowano na .html (ID tylko w meta, bez powtórzeń).")
